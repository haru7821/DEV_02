...
const View3D = (() => {
   ...
   * ① 상공 270° 선회 → ② 병상 필드 위를 가로지르는 저공 통과 → ③ 재상승
   *
   * 실내로 내려가지 않고 카메라를 내내 벽 높이(2.7m) 위에 두므로,
   * 어떤 도면에서도 벽·병상에 닿거나 바짝 붙는 구간이 생기지 않는다.
   *
  ...
  function dronePath() {
    ...
    const long = W >= H;
    const inset = 0.6;
    const A = long ? { x: W - inset, z: fz } : { x: fx, z: H - inset };
    const B = long ? { x: inset, z: fz } : { x: fx, z: inset };
    const dx = Math.sign(B.x - A.x), dz = Math.sign(B.z - A.z);
    // 주시점은 도면 안으로 묶는다 — 끝단에서 건물 너머 빈 배경을 보지 않게
    const clampIn = (v, max) => Math.min(max - 1, Math.max(1, v));

    // ① 상공 270° 선회 — 통과를 시작할 쪽 위에서 끝나도록 시작 각도를 잡는다
    const endA = Math.atan2(A.z - cz, A.x - cx);
    ...
    // ② 벽 위 통과 — 고도를 낮추며 실내를 훑고 지나간다.
    // 내려다보는 각은 화각(50°)의 절반보다 크게 잡는다 — 시선이 수평에 가까우면
    // 화면 위쪽이 건물 밖 빈 배경으로 차 버린다.
    phases.push({ w: 0.40, at(s) {
      const e = smooth(s);
      const y = orbitEnd.y + (SWEEP_Y - orbitEnd.y) * Math.min(1, e * 2.2);
      const x = A.x + (B.x - A.x) * e, z = A.z + (B.z - A.z) * e;
      const ahead = (y - 0.4) * 0.85;                 // 약 50° 내려봄
      return { p: V(x, y, z),
               t: V(clampIn(x + dx * ahead, W), 0.4, clampIn(z + dz * ahead, H)) };
    } });
